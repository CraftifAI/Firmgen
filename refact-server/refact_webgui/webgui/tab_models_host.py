...
class TabHostRouter(APIRouter):
    ...
    async def _tab_host_have_devices(self):
        try:
            devices = self._model_assigner.devices
            return Response(json.dumps(devices, indent=4) + "\n")
        except Exception as e:
            from refact_webgui.webgui.selfhost_webutils import log
            log(f"Error getting devices: {e}")
            # Return empty devices structure matching expected format
            empty_devices = {
                "cpu": {
                    "id": "cpu",
                    "name": "CPU",
            ...
            return Response(json.dumps(empty_devices, indent=4) + "\n")

    async def _tab_host_models_get(self):
        try:
            models_info = self._model_assigner.models_info
            model_assignment = self._model_assigner.model_assignment
            ...
            from refact_webgui.webgui.selfhost_webutils import log
            log(f"Error getting models: {e}")
            # Return empty models structure matching expected format
            empty_models = {
                "models": [],
                "model_assign": {}
            }
            return Response(json.dumps(empty_models, indent=4) + "\n")
