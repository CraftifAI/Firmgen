...
import { CHAT_URL, CHAT_PAYLOAD_DEBUG_URL } from "./consts";
...
export async function sendChat({
  ...
  const body = JSON.stringify({
    ...
    meta: {
      ...
      ...(esp32_projects_path && esp32_projects_path.trim()
        ? { esp32_projects_path: esp32_projects_path.trim() }
        : {}),
    },
  });
...
export type ChatPayloadDebugResponse = {
  payload: {
    messages?: unknown[];
    tools?: unknown[];
    tool_choice?: unknown;
    compression_strength?: unknown;
    prompt?: string;
  };
  model: string;
  model_id: string;
...
export async function getChatPayloadDebug({
  messages,
  model,
  port = 8001,
  apiKey,
  ...
  boost_reasoning,
  increase_max_tokens = false,
}: Omit<SendChatArgs, "stream" | "abortSignal">): Promise<ChatPayloadDebugResponse> {
  const body = JSON.stringify({
    messages,
    model: model,
    stream: false,
  ...
  const url = `http://127.0.0.1:${port}${CHAT_PAYLOAD_DEBUG_URL}`;
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to fetch payload debug: ${response.status} ${errorText}`);
  }
