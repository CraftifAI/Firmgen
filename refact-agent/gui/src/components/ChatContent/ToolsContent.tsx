...
const WORKSPACE_GENERATION_TOOLS = [
  "create_textdoc", "update_textdoc", "update_textdoc_regex",
  "create_knowledge", "create_memory_bank", "rm", "mv",
];
const WORKSPACE_PLANNING_TOOLS = [
  "strategic_planning", "task_list", "tree", "cat",
  "search_semantic", "search_pattern", "locate",
  "search_symbol_definition", "search_symbol_usages", "web", "knowledge",
];

function getToolCallStage(functionName: string, args: string): PipelineStage {
  const name = functionName.toLowerCase();
  let operation = "";
  try {
    const parsed = JSON.parse(args) as Record<string, unknown>;
    operation = ((parsed.operation as string) ?? "").toLowerCase();
  } catch { /* ignore */ }
  ...
  if (name === "esp32_build" &&
    ...
    return "COMPILING";
  if (name === "esp32_project" && operation.includes("create")) return "GENERATION";
  if (name === "esp32_config") return "GENERATION";
  if (name === "esp32_component" && (operation.includes("add") || operation.includes("remove")))
    return "GENERATION";
  if (WORKSPACE_GENERATION_TOOLS.includes(name)) return "GENERATION";
  if (WORKSPACE_PLANNING_TOOLS.includes(name)) return "PLANNING";
  return "PLANNING";
}
...
const stageBoxClass: Record<PipelineStage, string> = {
  PLANNING: styles.toolCallBox_planning,
  GENERATION: styles.toolCallBox_generation,
  COMPILING: styles.toolCallBox_compiling,
  FLASH: styles.toolCallBox_flash,
  MONITORING: styles.toolCallBox_monitoring,
};

const stageLabelClass: Record<PipelineStage, string> = {
  PLANNING: styles.toolCallStageLabel_planning,
  GENERATION: styles.toolCallStageLabel_generation,
  COMPILING: styles.toolCallStageLabel_compiling,
  FLASH: styles.toolCallStageLabel_flash,
  MONITORING: styles.toolCallStageLabel_monitoring,
};
type ResultProps = {
  children: string;
...
const StageLabel: React.FC<{ stage: PipelineStage }> = ({ stage }) => (
  <div className={classNames(styles.toolCallStageLabel, stageLabelClass[stage])}>
    <span className={styles.toolCallStageDot} />
    {STAGE_LABELS[stage]}
  </div>
);
...
const ToolMessage: React.FC<{
  ...
  const stage = useMemo(() => {
    return getToolCallStage(name, toolCall.function.arguments);
  }, [name, toolCall.function.arguments]);
  ...
  return (
    <Flex
      direction="column"
      className={classNames(styles.toolCallBox, stageBoxClass[stage])}
    >
          ...
          <Box py="2">
            {toolCalls.map((toolCall, i) => {
              ...
              const functionCalled =
                "```python\n" + name + "(" + argsString + ")\n```";
              const stage = getToolCallStage(name, toolCall.function.arguments);
...
const summaryColorClass: Record<PipelineStage, string> = {
  PLANNING: styles.toolSummary_planning,
  GENERATION: styles.toolSummary_generation,
  COMPILING: styles.toolSummary_compiling,
  FLASH: styles.toolSummary_flash,
  MONITORING: styles.toolSummary_monitoring,
};
...
const ToolUsageSummary = forwardRef<HTMLDivElement, ToolUsageSummaryProps>(
  (
    ...
    const stageColorCls = dominantStage ? summaryColorClass[dominantStage] : undefined;
    return (
      <AnimatedText as="div" weight="light" size="1" animating={waiting}>
        <Flex gap="2" align="end" onClick={onClick} ref={ref} my="2" className={stageColorCls}>
          <Flex
            gap="1"
            align="start"
            direction="column"
