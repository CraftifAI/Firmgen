...
export const historySlice = createSlice({
  ...
  reducers: {
    ...
    setChatProjectById: (
      ...
      const { chatId, projectId, projectName, esp32_projects_path } =
        action.payload;
      if (!(chatId in state)) return;
      if (projectId == null) {
        delete state[chatId].project_id;
        delete state[chatId].project_name;
        delete state[chatId].esp32_projects_path;
        return;
      }
      state[chatId].project_id = projectId;
      if (projectName !== undefined) state[chatId].project_name = projectName;
      if (esp32_projects_path !== undefined)
        state[chatId].esp32_projects_path = esp32_projects_path;
    },
  },
  selectors: {
