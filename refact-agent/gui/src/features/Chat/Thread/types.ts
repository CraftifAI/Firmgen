...
export type ChatThread = {
  ...
  mode?: LspChatMode;
  /** Absolute path: ESP-IDF apps created in this chat go here (overrides esp32_tools.yaml projects_path). */
  esp32_projects_path?: string;
  /** Workspace project this chat belongs to (sidebar Projects). */
  project_id?: string;
  project_name?: string;
  ...
  subchat_usage_by_tool?: Record<string, SubchatToolUsage>;
};
...
export type Chat = {
  ...
  /** Processed file content to append only to the next API request (not stored or displayed). */
  pendingProcessedContext: string | null;
  /** Display version of last user message (short, no file content) when server may echo full content. */
  lastUserMessageDisplayContent: string | null;
};
