...
const createInitialState = ({
  ...
  return {
    ...
    pendingProcessedContext: null,
    lastUserMessageDisplayContent: null,
  };
};
...
export const chatReducer = createReducer(initialState, (builder) => {
  ...
  builder.addCase(setPendingProcessedContext, (state, action) => {
    state.pendingProcessedContext = action.payload;
  });

  builder.addCase(setLastUserMessageDisplayContent, (state, action) => {
    state.lastUserMessageDisplayContent = action.payload;
  });
  ...
  builder.addCase(newChatAction, (state, action) => {
    ...
    if (action.payload && "esp32_projects_path" in action.payload) {
      next.thread.esp32_projects_path = action.payload.esp32_projects_path;
    }
    if (action.payload && "project_id" in action.payload) {
      next.thread.project_id = action.payload.project_id;
    }
    return next;
  });
  ...
  builder.addCase(chatResponse, (state, action) => {
    ...
    // Swap out the server's echoed full message (with file text) for the shorter display version
    if (state.lastUserMessageDisplayContent && messages.length > 0) {
      const lastUserIndex = messages.reduce<number>(
        (idx, msg, i) => (isUserMessage(msg) ? i : idx),
        -1,
      );
      if (lastUserIndex >= 0) {
        const msg = messages[lastUserIndex] as UserMessage;
        messages[lastUserIndex] = {
          ...msg,
          content: state.lastUserMessageDisplayContent,
        } as UserMessage;
      }
    }
    ...
    // Accumulate subchat token usage separately from main agent tokens
    if (isSubchatResponse(action.payload) && action.payload.subchat_usage) {
      const prev = state.thread.subchat_usage_total ?? { prompt_tokens: 0, completion_tokens: 0 };
      state.thread.subchat_usage_total = {
        prompt_tokens: prev.prompt_tokens + action.payload.subchat_usage.prompt_tokens,
        completion_tokens: prev.completion_tokens + action.payload.subchat_usage.completion_tokens,
      };

      const toolCallId = action.payload.tool_call_id;
      if (toolCallId && state.thread.messages) {
        ...
        if (!state.thread.subchat_usage_by_tool) {
          state.thread.subchat_usage_by_tool = {};
        }

        const prevToolUsage = state.thread.subchat_usage_by_tool[toolName] ?? {
          prompt_tokens: 0,
          completion_tokens: 0,
          invocations: 0,
        };

        state.thread.subchat_usage_by_tool[toolName] = {
          prompt_tokens: prevToolUsage.prompt_tokens + action.payload.subchat_usage.prompt_tokens,
          completion_tokens: prevToolUsage.completion_tokens + action.payload.subchat_usage.completion_tokens,
          invocations: prevToolUsage.invocations + 1,
        };
      }
    }
  ...
  builder.addCase(setChatProject, (state, action) => {
    const { chatId, projectId, projectName, esp32_projects_path } = action.payload;
    const apply = (thread: Draft<ChatThread>) => {
      if (projectId == null) {
        delete thread.project_id;
        delete thread.project_name;
        delete thread.esp32_projects_path;
        return;
      }
      thread.project_id = projectId;
      if (projectName !== undefined) thread.project_name = projectName;
      if (esp32_projects_path !== undefined) thread.esp32_projects_path = esp32_projects_path;
    };

    if (state.thread.id === chatId) {
      apply(state.thread as Draft<ChatThread>);
      return;
    }
    if (chatId in state.cache) {
      apply(state.cache[chatId] as Draft<ChatThread>);
    }
  });
