...
export const setPendingProcessedContext = createAction<string | null>(
  "chatThread/setPendingProcessedContext",
);

export const setLastUserMessageDisplayContent = createAction<string | null>(
  "chatThread/setLastUserMessageDisplayContent",
);
...
export const setChatProject = createAction<{
  chatId: string;
  projectId: string | null;
  projectName?: string;
  esp32_projects_path?: string;
}>("chatThread/setChatProject");
...
>(
  ...
  ({ messages, chatId, mode, checkpointsEnabled }, thunkAPI) => {
    ...
    const pendingContext = state.chat.pendingProcessedContext;
    thunkAPI.dispatch(setPendingProcessedContext(null));
    ...
    if (pendingContext && pendingContext.trim()) {
      ...
      if (lastUserIndex >= 0) {
        const lastMsg = messages[lastUserIndex] as UserMessage;
        const content = lastMsg.content;
        const augmentedContent =
          typeof content === "string"
            ? content + "\n\n" + pendingContext
            : [...content, { type: "text" as const, text: pendingContext }];
    ...
    // stops the stream
    const onlyDeterministicMessages = checkForToolLoop(messagesToSend);

    const messagesForLsp = formatMessagesForLsp(messagesToSend);
    ...
    const esp32_projects_path = thread?.esp32_projects_path;
    ...
    return sendChat({
      ...
      .then(async (response) => {
        ...
        const chunkBuffer: ChatResponse[] = [];
        ...
        const scheduleFlush = () => {
          ...
          // Use requestAnimationFrame for smooth updates (syncs with browser refresh)
        ...
        const onChunk = (json: Record<string, unknown>) => {
          const chunk = json as ChatResponse;
          chunkBuffer.push(chunk);

          // For critical chunks (like tool calls or finish reasons), flush immediately
          const isCritical =
            (isChatResponseChoice(chunk) && chunk.choices && chunk.choices[0]?.delta && "tool_calls" in chunk.choices[0].delta) ||
            (isChatResponseChoice(chunk) && chunk.choices && chunk.choices[0]?.finish_reason) ||
            (chunk as any).role === "tool" ||
            (chunk as any).role === "diff";

          if (isCritical || chunkBuffer.length >= 5) {
            // Flush immediately for critical chunks or when buffer is full
            flushChunks();
          } else {
        ...
        return consumeStream(reader, thunkAPI.signal, onAbort, onChunk)
          .then(() => {
            // Flush any remaining chunks when stream ends
            flushChunks();
          })
          .catch((err) => {
            // Flush before error handling
            flushChunks();
            throw err;
          });
      })
