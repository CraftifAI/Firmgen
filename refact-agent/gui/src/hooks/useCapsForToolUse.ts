...
export function useCapsForToolUse() {
  ...
  const usableModelsForPlan = useMemo(() => {
    ...
    return usableModels.map((model) => {
      // Show short name in UI (e.g. "gpt-5" instead of "local_openai_proxy/gpt-5")
  ...
  useEffect(() => {
    if (usableModelsForPlan.length > 0) {
      const modelIds: string[] = usableModelsForPlan.map((elem) => elem.value);
      const toChange =
        modelIds.find((id) => id === currentModel) ?? modelIds[0];
