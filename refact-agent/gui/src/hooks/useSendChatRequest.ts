...
export const useSendChatRequest = () => {
  ...
  const submit = useCallback(
    ({
      ...
      void (async () => {
        const projectRoot = thread.esp32_projects_path?.trim();
        if (projectRoot && attachedImages.length > 0) {
          const files: File[] = [];
          attachedImages.forEach((img, i) => {
            if (typeof img.content !== "string") return;
            const name = uniqueArchiveFilename(img.name || "paste.png", i);
            const f = dataUrlToFile(img.content, name, img.type);
            if (f) files.push(f);
          });
          if (files.length > 0) {
            try {
              await uploadProjectSources(projectRoot, files, lspPort);
            } catch (e) {
              dispatch(
                setInformation(
                  `Could not save pasted images to project Sources: ${
