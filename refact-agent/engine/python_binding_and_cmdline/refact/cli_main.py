...
def save_chat_history():
    ...
    import re
    history_dir = os.path.expanduser("~/.cache/refact/")
    os.makedirs(history_dir, exist_ok=True)

    chat_id = None
    try:
        chat_id = getattr(cli_settings.args, "chat_id", None)
    except Exception:
        chat_id = None
    if not chat_id:
        import time
        chat_id = f"unknown_{int(time.time())}"
    # Ensure chat_id is filesystem-safe
    chat_id_safe = re.sub(r'[^a-zA-Z0-9_.-]', '_', str(chat_id))

    history_file = os.path.join(history_dir, f"chat_history__{chat_id_safe}.json")
    serializable_messages = []
    for msg in cli_streaming.streaming_messages:
...
async def chat_main():
    ...
    # New: forward additional options to the refact-lsp binary
    parser.add_argument('--board-definition', type=str, default='', help="Board definition ID to pre-load (forwarded to refact-lsp)")
    parser.add_argument('--platform', type=str, default='', help="Platform (e.g., esp32) to enable platform-specific tools (forwarded to refact-lsp)")
    ...
    # forward board-definition and platform flags to the underlying binary if provided
    if getattr(args_parsed, "board_definition", ""):
        refact_args.extend(["--board-definition", args_parsed.board_definition])
    if getattr(args_parsed, "platform", ""):
        refact_args.extend(["--platform", args_parsed.platform])
