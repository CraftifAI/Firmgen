...
pub struct AtCommandsContext {
    ...
    /// Cache of file paths already read this session → (mtime_secs, line_count).
    /// `tool_cat` checks this to emit a short "already in context" note instead of
    /// re-injecting the full file content, saving substantial context tokens.
    pub file_read_cache: HashMap<String, (u64, usize)>,

    /// Per-chat override for where ESP-IDF projects are created (`esp32_project` tool).
    /// `None` means fall back to `projects_path` from `esp32_tools.yaml`.
    pub esp32_projects_path: Option<PathBuf>,
}
