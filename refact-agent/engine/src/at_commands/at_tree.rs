...
impl AtCommand for AtTree {
    ...
    async fn at_execute(
        ...
        let filtered_paths: Vec<PathBuf> = paths_from_anywhere.into_iter()
            ...
            // Opt 3: Collapse managed_components/ and build/ from tree output.
            // These are auto-generated/downloaded directories; showing their internals wastes
            // 2-4K tokens in large projects. The tree will show a summary line instead.
            .filter(|path| {
                let s = path.to_string_lossy();
                !s.contains("/managed_components/") && !s.contains("/build/")
            })
            .collect();
