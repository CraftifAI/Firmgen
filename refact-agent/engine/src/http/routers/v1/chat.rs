...
async fn _chat(
    ...
    if let Some(ref s) = chat_post.meta.esp32_projects_path {
        let t = s.trim();
        if !t.is_empty() {
            ccx.esp32_projects_path = Some(PathBuf::from(t));
        }
    }
...
pub async fn handle_v1_chat_payload_debug(
    Extension(gcx): Extension<SharedGlobalContext>,
    body_bytes: hyper::body::Bytes,
) -> Result<Response<Body>, ScratchError> {
    let mut chat_post: ChatPost = serde_json::from_slice::<ChatPost>(&body_bytes).map_err(|e| {
        tracing::warn!("chat-payload-debug handler cannot parse input:\n{:?}", body_bytes);
        ScratchError::new(StatusCode::BAD_REQUEST, format!("JSON problem: {}", e))
    })?;
    ...
    tracing::info!("chat-payload-debug: chat_mode {:?}", chat_post.meta.chat_mode);
    ...
    // Validate messages (same as _chat)
    let re = regex::Regex::new(r"\{.*?\}").unwrap();
    for message in messages.iter_mut() {
        if !model_rec.supports_multimodality {
            if let ChatContent::Multimodal(content) = &message.content {
        ...
        if let Some(tool_calls) = &mut message.tool_calls {
            for call in tool_calls {
                ...
                if let Some(captures) = re.captures(args_input) {
                    let corrected_arg = captures.get(0).unwrap().as_str();
                    if let Ok(args_corrected_json) = serde_json::from_str::<serde_json::Value>(corrected_arg) {
                        if let Ok(args_corrected) = serde_json::to_string(&args_corrected_json) {
                            call.function.arguments = args_corrected;
                        }
                    }
                }
            }
    ...
    // Create scratchpad (same as _chat)
    let mut scratchpad = crate::scratchpads::create_chat_scratchpad(
        gcx.clone(),
        &mut chat_post,
        tools,
    ...
    // Get the final payload by calling prompt()
    let mut sampling_params = chat_post.parameters.clone();
    let prompt_result = scratchpad.prompt(ccx_arc.clone(), &mut sampling_params).await
        .map_err(|e| ScratchError::new(StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to generate payload: {}", e)))?;

    // Parse the payload - for passthrough models it's "PASSTHROUGH " + JSON
    let payload_json: serde_json::Value = if prompt_result.starts_with("PASSTHROUGH ") {
        let json_str = prompt_result.strip_prefix("PASSTHROUGH ").unwrap();
        serde_json::from_str(json_str).map_err(|e| {
            ScratchError::new(StatusCode::INTERNAL_SERVER_ERROR, format!("Failed to parse passthrough payload: {}", e))
        })?
    } else {
        // For non-passthrough models, return the prompt as-is in a structured format
        serde_json::json!({
            "prompt": prompt_result,
            "model": chat_post.model,
            "endpoint_style": if model_rec.base.endpoint_style == "hf" { "hf" } else { "generic" },
        })
    };
    ...
    let response = serde_json::json!({
        ...
        "model": chat_post.model,
        "model_id": model_rec.base.id,
        "supports_tools": model_rec.supports_tools,
        "endpoint_style": model_rec.base.endpoint_style,
        "sampling_parameters": {
