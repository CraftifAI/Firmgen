...
pub async fn do_i_need_to_reload_vecdb(
    ...
    let mut consts = {
        VecdbConstants {
            ...
            tokenizer: None,
            // Use 1500 tokens as optimal chunk size for embeddings (well below 8000 limit)
            // This ensures better semantic search quality and prevents token limit errors
...
pub async fn vecdb_background_reload(
    ...
    loop {
        ...
        if need_reload && consts.is_some() {
            ...
            // Reduced max_delay_ms: worst-case retry chain is now ~250 ms
            // (10→20→40→80→100) rather than the previous ~1150 ms (10→20→40→80→1000).
...
impl VecDb {
    /// Separator used when packing/unpacking multiple workspace roots in `cmdline.workspace_folder`.
    /// Must match the separator used in `lsp_like_handlers::workspace_folder_sep()`.
    /// Windows paths already contain `:` (drive letter), so we use `|` there.
    ...
    pub async fn init(
        ...
        // Canonicalize workspace folder to ensure consistent hashing.
        // This ensures Python CLI and GUI produce the same hash for the same workspace
        // even if paths have symlinks or different representations.
        // Handle both single folder and multiple folders (joined with platform-specific separator).
        let workspace_folder = if cmdline.workspace_folder.is_empty() {
            String::new()
        } else {
            // Split by platform separator, canonicalize each folder, then rejoin
            let folders: Vec<String> = cmdline.workspace_folder.split(Self::workspace_folder_sep())
                .map(|p| {
                    let canonical = crate::files_correction::canonical_path(p);
                    canonical.to_string_lossy().to_string()
                })
                .collect();
            folders.join(Self::workspace_folder_sep())
        };
        let emb_table_name = crate::vecdb::vdb_emb_aux::create_emb_table_name(&vec![workspace_folder.clone()]);
        let workspace_hash = crate::vecdb::vdb_emb_aux::create_workspace_hash(&vec![workspace_folder]);
        let handler = VecDBSqlite::init(cache_dir, &constants.embedding_model.base.name, constants.embedding_model.embedding_size, &emb_table_name, &workspace_hash).await?;
