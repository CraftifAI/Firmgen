...
impl FileSplitter {
    ...
    /// Maximum file size to process for vectorization (10 MB)
    ...
    pub async fn vectorization_split(&self, doc: &Document,
        ...
        // Skip files that are too large to avoid excessive memory usage
        if text.len() > Self::MAX_FILE_SIZE_BYTES {
            tracing::warn!("Skipping file {} for vectorization: size {} bytes exceeds limit of {} bytes",
                path.display(), text.len(), Self::MAX_FILE_SIZE_BYTES);
            return Ok(Vec::new());
        }
