...
/// Run diagnostics on the VecDB and log the results
pub async fn vecdb_run_diagnostics(vecdb: &VecDb) {
    let handler = vecdb.vecdb_handler.lock().await;
    match handler.diagnose().await {
        Ok(report) => {
            info!("=== VecDB Diagnostics ===\n{}", report);
        }
        Err(e) => {
            warn!("VecDB diagnostics failed: {}", e);
        }
    }
    
    match handler.get_stats().await {
        Ok(stats) => {
            info!("VecDB stats: cache_entries={}, cache_duplicates={}, vector_entries={}, duplicate_vectors={}, unique_files={}",
                stats.cache_entries, stats.cache_duplicates, stats.vector_entries, stats.duplicate_vectors, stats.unique_files);
        }
        Err(e) => {
            warn!("VecDB get_stats failed: {}", e);
        }
    }
}
...
pub async fn initialize_vecdb_with_context(
    ...
    // Run diagnostics to log current DB state
    vecdb_run_diagnostics(&vec_db).await;
