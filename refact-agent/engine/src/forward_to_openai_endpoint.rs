...
pub async fn forward_to_openai_style_endpoint(
    ...
    if let Some(reasoning_effort) = sampling_parameters.reasoning_effort.clone() {
        ...
        if model_rec.name.starts_with("gpt-5") {
            data["temperature"] = serde_json::Value::from(1.0);
        } else {
...
pub async fn forward_to_openai_style_endpoint_streaming(
    ...
    // Retry logic for local endpoints that may be temporarily unavailable
    let max_retries = 3;
    ...
    for attempt in 1..=max_retries {
        ...
        match EventSource::new(builder) {
            Ok(event_source) => {
                if attempt > 1 {
                    tracing::info!("Successfully connected to {} after {} attempts", model_rec.endpoint, attempt);
                }
            ...
            Err(e) => {
                ...
                if attempt < max_retries {
                    let error_msg = format!("Connection to {} failed (attempt {}/{}): {}", 
                        model_rec.endpoint, attempt, max_retries, last_error.as_ref().unwrap());
    ...
    Err(format!("Failed to connect to {} after {} attempts: {}", 
        model_rec.endpoint, max_retries, last_error.unwrap()))
}
