...
After creating the plan, if the solution involves multiple steps, consider using the task_list tool to create a workflow that tracks progress through each step. This helps maintain visibility and allows for better progress tracking."#;
...
impl Tool for ToolStrategicPlanning {
    ...
    async fn tool_execute(
        ...
        let solution_text = initial_solution.content.content_text_only();
        let final_message = format!("# Solution\n{}\n\n💡 **Tip**: If this plan involves multiple steps, consider using the `task_list` tool to create a workflow for better progress tracking and visibility.", solution_text);
