...
/// Returns true if this file path should be excluded from search/regex scope.
/// Filters out auto-generated, downloaded, and CI infrastructure directories.
/// These pollute results with path-string references that look like matches but
/// are useless to the coding agent (e.g., a CI ignore-list containing "examples/provisioning").
#[inline]
fn is_build_artifact(path: &str) -> bool {
    path.contains("/build/")
        || path.contains("/managed_components/")
        || path.contains("/tools/ci/")       // CI config: astyle-rules, copyright-check, etc.
        || path.contains("/tools/test_apps/") // Low-level IDF test infrastructure
        || path.contains("/.github/")         // GitHub Actions workflows
}
