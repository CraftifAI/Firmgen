...
impl Tool for ToolC2000Build {
    ...
    fn tool_description(&self) -> ToolDesc {
        ToolDesc {
            ...
            agentic: true,
            experimental: false,
            description: "Build TI C2000 microcontroller project for specific configuration (RAM/FLASH) with automatic dependency resolution and error reporting. Use this after creating a project to compile it for your target device (LaunchPad, controlCARD, etc.)".to_string(),
            parameters: vec![
                ToolParam {
    ...
    async fn tool_execute(
        ...
        // Execute build command with timeout to prevent connection timeouts
        let build_timeout_secs = 600; // 10 minutes timeout for builds
        let output = tokio::time::timeout(
            Duration::from_secs(build_timeout_secs),
            build_cmd.output()
        ).await
        .map_err(|_| format!("Build command timed out after {} seconds. The build may still be running in the background.", build_timeout_secs))?
        ...
        // Add any stdout information (limit size to prevent huge responses)
        if !output.stdout.is_empty() {
            let stdout_msg = String::from_utf8_lossy(&output.stdout);
            // Limit output to last 10000 characters to prevent overwhelming the connection
            let display_output = if stdout_msg.len() > 10000 {
                format!("... (truncated, showing last 10000 chars)\n{}", 
                    &stdout_msg[stdout_msg.len() - 10000..])
            } else {
