...
pub async fn run_tools_remotely(
    ...
    let esp32_projects_path = {
        let cx = ccx.lock().await;
        cx.esp32_projects_path
            .as_ref()
...
pub async fn run_tools(
    ...
    for t_call in last_msg_tool_calls.iter() {
        ...
        let tool_name = t_call.function.name.as_str();
        let track_generic_progress =
            !chat_id.is_empty() && !progressbar::tool_manages_own_progress(tool_name);
        ...
        if track_generic_progress {
            progressbar::record_tool_start(
                &chat_id,
                &t_call.id,
                tool_name,
                &operation,
        ...
        if track_generic_progress {
            let summary = summarize_tool_result_for_progress(&t_call.id, &tool_execute_results);
            progressbar::record_generic_tool_success(
                &chat_id,
                &t_call.id,
                tool_name,
                &operation,
                ...
                ContextEnum::ChatMessage(m) => {
                    if (m.role == "tool" || m.role == "diff") && m.tool_call_id == t_call.id {
                        ...
                        if !m.tool_call_id.is_empty() {
                            warn!("tool use {}: unexpected non-empty tool_call_id '{}' on non-tool message", &t_call.function.name, m.tool_call_id);
                        }
        ...
        if !have_answer {
            warn!("tool use {}: tool returned results but none had role='tool' with matching tool_call_id '{}'", &t_call.function.name, t_call.id);
            generated_tool.push(tool_answer_err(
                format!("Tool '{}' did not return a proper result", &t_call.function.name),
                t_call.id.to_string(),
            ));
        }
    }
    ...
    if tokens_for_rag < MIN_RAG_CONTEXT_LIMIT {
        warn!("There are tool results, but tokens_for_rag={tokens_for_rag} is very small. Returning truncation warning instead of silently dropping results.");
        // Instead of silently dropping all tool results, return a warning message
        // so the user/agent knows what happened
        let warning_messages: Vec<ChatMessage> = last_msg_tool_calls.iter().map(|t_call| {
            ChatMessage {
                role: "tool".to_string(),
                content: ChatContent::SimpleText(
                    "Tool result truncated: context window is nearly full. Please start a new chat or reduce conversation length.".to_string()
                ),
                tool_calls: None,
        ...
        return Ok((warning_messages, true));
    }
...
/// Short summary for progress store (first tool message body, truncated).
fn summarize_tool_result_for_progress(tool_call_id: &str, results: &[ContextEnum]) -> String {
    for msg in results {
        if let ContextEnum::ChatMessage(m) = msg {
            if (m.role == "tool" || m.role == "diff") && m.tool_call_id == tool_call_id {
                let t = m.content.content_text_only();
