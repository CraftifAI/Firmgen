        ...
        if f_type.starts_with("image/") {
            ...
            match get_file_text_from_memory_or_disk(gcx.clone(), &PathBuf::from(p)).await {
                Ok(text) => {
                    ...
                    // ── Context caching (Opt 1) ──────────────────────────────────────
                    // Only applies when reading the whole file (no line range requested).
                    // Reads the file's mtime; if we've seen this exact mtime before, the
                    // file is unchanged → emit a short note instead of re-injecting it.
                    if line_range.is_none() && arg_symbols.is_empty() {
                        let mtime_secs: u64 = std::fs::metadata(p)
                        ...
                        let cached = ccx.lock().await.file_read_cache.get(p).cloned();
                        if let Some((cached_mtime, cached_lines)) = cached {
                            if cached_mtime == mtime_secs {
                                // File unchanged since last read — skip re-injection
                                not_found_messages.push(format!(
                                    "{}  ← already in context ({} lines, unchanged)",
                                    p, cached_lines
                                ));
                                filenames_present.push(p.clone());
                                continue;
                        ...
                        // First read or file changed — update cache
                        ccx.lock().await.file_read_cache.insert(p.clone(), (mtime_secs, total_lines));
                    }
                    // ─────────────────────────────────────────────────────────────────
