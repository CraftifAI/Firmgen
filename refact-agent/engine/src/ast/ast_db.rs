    ...
    for (uindex, usage) in definition.usages.iter().enumerate() {
        ...
        for to_resolve_unstripped in &usage.targets_for_guesswork {
            ...
            if magnifying_glass_pairs.len() == 0 {
                ...
                fn generate_combinations(substitutions: &[Vec<String>], index: usize, current: Vec<String>, limit: usize) -> Vec<Vec<String>> {
                    ...
                    for substitution in &substitutions[index] {
                        ...
                        result.extend(generate_combinations(substitutions, index + 1, new_current, limit - result.len()));
                    }
                    result
                }
                let intermediate_results = generate_combinations(&substitutions_of_each_pair, 0, Vec::new(), MAX_COMBINATIONS);
