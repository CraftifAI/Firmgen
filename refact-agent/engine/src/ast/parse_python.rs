...
/// Maximum number of resolution passes to prevent infinite loops on pathological code
const MAX_RESOLUTION_PASSES: usize = 50;
...
pub fn py_parse(code: &str) -> ContextPy
{
    ...
    loop {
        ...
        if pass_n >= MAX_RESOLUTION_PASSES {
            tracing::warn!("py_parse: hit maximum resolution passes ({}), stopping to prevent infinite loop", MAX_RESOLUTION_PASSES);
            break;
        }
        cx.ap.usages.clear();
