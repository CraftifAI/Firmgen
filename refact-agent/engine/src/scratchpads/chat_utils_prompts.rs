...
"
    ...
    if system_prompt.contains("%IDF_EXAMPLES_SCOPE%") {
        use crate::tools::esp32_tools::global_state;
        let scope = match global_state::get_config().await {
            Ok(config) => {
                let basename = std::path::Path::new(&config.esp_idf_path)
                    .file_name()
                    .and_then(|n| n.to_str())
                    .unwrap_or("esp-idf");
                format!("{}/examples/", basename)
            }
            Err(_) => "esp-idf/examples/".to_string(),
        };
        system_prompt = system_prompt.replace("%IDF_EXAMPLES_SCOPE%", &scope);
    }
