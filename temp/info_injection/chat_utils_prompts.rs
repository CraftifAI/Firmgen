...
"
    ...
    if system_prompt.contains("%ESP32_SESSION_CONTEXT%") {
        use crate::tools::esp32_tools::global_state;
        let context = global_state::get_session_summary(200).await;
        system_prompt = system_prompt.replace("%ESP32_SESSION_CONTEXT%", &format!("\nESP32 Session Context:\n{}", context));
    }

    // Optional: include richer board + config context if placeholder is present and board_id is set.
...
pub async fn prepend_the_right_system_prompt_and_maybe_more_initial_messages(
    ...
    match chat_meta.chat_mode {
        ChatMode::EXPLORE | ChatMode::AGENT | ChatMode::NO_TOOLS => {
            ...
            // If ESP32 tools are available in this chat, attach context so the LLM can see
            // config, esp32_tools.yaml overview, and (when selected) board constraints.
            // Config + esp32_tools.yaml are ALWAYS injected; board context is added when a board is selected.
            ...
            if has_esp32_tools {
                ...
                if !ctx_files.is_empty() {
                    for f in &ctx_files {
                        tracing::info!(
                            "Adding ESP32 ContextFile ({} chars, name={})",
                            f.file_content.len(),
                            f.file_name
                        );
                    }
                    ...
                    // Insert right after the system message, before user content.
