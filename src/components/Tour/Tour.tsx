...
export const Tour: React.FC<TourProps> = ({ page }) => {
  ...
  const historyTourPage = groupSelectionEnabled ? "history" : "chat";
  ...
  useEffect(() => {
    ...
    if (
      state.type === "in_progress" &&
      step === 6 &&
      (groupSelectionEnabled ? page === "history" : page === "chat")
    ) {
  ...
  }, [state.type, step, page, dispatch, groupSelectionEnabled]);
  ...
  return (
    <>
      ...
      <TourBubble
        ...
        containerWidth={chatWidth}
        onPage={"chat"}
        onNext={groupSelectionEnabled ? openHistory : undefined}
        page={page}
        bubbleContainerStyles={{
          maxWidth: 550,
          alignSelf: "start",
