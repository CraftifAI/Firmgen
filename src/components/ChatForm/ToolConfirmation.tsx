...
const PatchConfirmation: React.FC<PatchConfirmationProps> = ({
  ...
  let extractedFileNameFromPath = "file";
  try {
    const rawArgs = toolCalls[0].function.arguments;
    if (rawArgs) {
      const parsed = JSON.parse(rawArgs) as { path?: string; tickets?: string };
      const path = parsed?.path;
      if (typeof path === "string" && path.length > 0) {
        const parts = path.split(/[/\\]/);
        extractedFileNameFromPath = parts[parts.length - 1] ?? path;
      }
    }
  } catch {
    // Malformed or missing arguments (e.g. uncalled tool on resume) – use fallback
  }
