...
export type Config = {
  ...
  tabbed?: boolean;
  lspUrl?: string;
  /** Base URL for file upload/parse API (refactapi). If not set, falls back to lspUrl. */
  uploadApiUrl?: string;
  dev?: boolean;
  ...
  features?: {
    ...
    ast?: boolean;
    images?: boolean;
    embedded?: boolean; // Enable embedded development panels (UART, Command Log, VecDB Status)
  };
  keyBindings?: {
...
const initialState: Config = {
  host: "web",
  lspPort: Number(__REFACT_LSP_PORT__) || 8486,
  apiKey: null,
  features: {
    statistics: true,
    vecdb: true,
    ast: true,
    images: true,
    embedded: false, // Default to false - enable via environment variable or settings
  },
  themeProps: {
    appearance: "dark",
  },
...
export const reducer = createReducer<Config>(initialState, (builder) => {
  ...
  builder.addCase(updateConfig, (state, action) => {
    ...
    state.uploadApiUrl = action.payload.uploadApiUrl ?? state.uploadApiUrl;
...
export const selectEmbedded = createSelector(
  selectFeatures,
  (features) => features?.embedded ?? false,
);
