...
export type SubchatResponse = {
  add_message?: ChatResponse;          // optional: absent on usage-only events
  subchat_id: string;
  tool_call_id: string;
  subchat_usage?: {                    // present on the final usage-summary event
...
export function isSubchatResponse(json: unknown): json is SubchatResponse {
  ...
  if (!("tool_call_id" in json)) return false;
  // must have either add_message (message event) or subchat_usage (usage event)
  return "add_message" in json || "subchat_usage" in json;
}
