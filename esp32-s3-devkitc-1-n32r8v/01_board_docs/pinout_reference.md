...
### PWM/LEDC (Example)
- **LEDC Channel 0**: GPIO38 (on-board RGB LED for v1.1; v1.0 uses GPIO48)
- **LEDC Channel 1**: GPIO1, GPIO2, GPIO4, etc. (any safe GPIO)

### Timer (Example)
- **Timer 0**: Available via LEDC on any GPIO
- **Timer 1**: Available via LEDC on any GPIO
...
## N32R8V Variant Specifics

- **Flash Size**: 32MB (SPI/QSPI)
- **PSRAM Size**: 8MB (SPI/QSPI)
- **Partition Table**: Use appropriate scheme for 32MB flash
- **Recommended**: OTA partition scheme with larger app partitions (4-8MB)
- **Advantages**: More space for OTA updates, larger applications, more NVS storage
