...
This command creates a new component, which will have a minimum set of files necessary for building. The `-C` option can be used to specify the directory the component will be created in. For more information about components see the [Component CMakeLists Files](../build-system.html#component-directories).
...
> - Run the main build tool ([Ninja](https://ninja-build.org) or GNU Make). By default, the build tool is automatically detected but it can be explicitly set by passing the `-G` option to `idf.py`.
...
To enable autocompletion for `idf.py`, use the `export` command ([setting up the environment for Windows](../../get-started/windows-start-project.html#get-started-set-up-env), [Linux or macOS](../../get-started/linux-macos-start-project.html#get-started-set-up-env-linux-macos)). Autocompletion is initiated by pressing the TAB key. Type `idf.py -` and press the TAB key to autocomplete options.
...
This command prints the contents of the `otadata` partition which stores the information about the currently selected OTA app slot. Refer to [Over The Air Updates (OTA)](../../api-reference/system/ota.html) for more about the `otadata` partition.
...
The `version` field represents the JSON schema version for CMake Presets. In this example it is set to `3` to match the schema supported by ESP-IDF's minimal supported CMake version. If you are using a newer CMake version, you can increase the `version` field accordinglyâsee [CMake Presets](https://cmake.org/cmake/help/latest/manual/cmake-presets.7.html).
...
For a complete example, see the [Multiple Build Configurations Example](https://github.com/espressif/esp-idf/blob/35e0c8a3/examples/build_system/cmake/multi_config/README.md) .
...
Note that some older versions of [CCache](https://ccache.dev/) may exhibit bugs on some platforms, so if files are not rebuilt as expected, try disabling [CCache](https://ccache.dev/) and rebuilding the project. To enable [CCache](https://ccache.dev/) by default, set the `IDF_CCACHE_ENABLE` environment variable to a non-zero value.
...
For example, let's have a file custom\_flash.txt:
...
A further example of how this argument file can be used, e.g., creating configuration profile files via @filename, is in the [Multiple Build Configurations Example](https://github.com/espressif/esp-idf/blob/35e0c8a3/examples/build_system/cmake/multi_config/README.md) .
...
- `global_action_callbacks`: A list of functions called once before any task execution. Each global action callback function accepts three arguments:
  ...
  > - `ctx` â The [click context](https://click.palletsprojects.com/en/stable/api/#context)
  > - `global_args` â All available global arguments
  > - `tasks` â The list of tasks to be executed. Task refer to the action / sub-command used with idf.py
- `actions`: A dictionary of new subcommands. Each action has a `callback` function and may also include `options`, `arguments`, `dependencies`, etc. Each action callback function accepts three to four arguments:
  ...
  > - `subcommand_name` â the name of the command (useful if multiple commands share the same callback)
  > - `ctx` â the [click context](https://click.palletsprojects.com/en/stable/api/#context)
  > - `global_args` â all available global arguments,
  > - `**action_args` â (optional) arguments passed to the action
