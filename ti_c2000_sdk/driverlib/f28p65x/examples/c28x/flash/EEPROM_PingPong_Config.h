...
// 1. Which Flash Bank to use for EEPROM Emulation
// 2. EEPROM Bank Size
// 3. EEPROM Page Size
// 4. Flash Sector Size
// 5. Page_Mode or EIGHT_BYTE_MODE
//
...
// Define size of EEPROM Pages in 16-bit words
#define DATA_SIZE 64
...
// DATA_SIZE specified by user (measured in 16-bit words)
#define EEPROM_PAGE_DATA_SIZE (DATA_SIZE + 3 - ((DATA_SIZE + 3) % 4))

// Pointer Initialization
// Note: SECTOR_NUMBERS needs to be in order from least to greatest
#define RESET_BANK_POINTER Bank_Pointer = (uint16 *) (FLASH_BANK_SELECT + ((uint32)FIRST_AND_LAST_SECTOR[EEPROM_ACTIVE_UNIT][0] * FLASH_SECTOR_SIZE))
#define RESET_PAGE_POINTER Page_Pointer = (uint16 *) (FLASH_BANK_SELECT + ((uint32)FIRST_AND_LAST_SECTOR[EEPROM_ACTIVE_UNIT][0] * FLASH_SECTOR_SIZE)) + 8

// Define end address of last sector to be used in EEPROM Emulation
#define END_OF_SECTOR (FLASH_BANK_SELECT + ((uint32)FIRST_AND_LAST_SECTOR[EEPROM_ACTIVE_UNIT][1] * FLASH_SECTOR_SIZE) + (FLASH_SECTOR_SIZE - 1));
...
// Define Flash sector size for devices
#define F28P65x_FLASH_SECTOR_SIZE 0x400

// Define number of Flash sectors for devices
#define F28P65x_NUM_FLASH_SECTORS 128

// Function Prototypes
extern int EEPROM_Config_Check();
extern void EEPROM_GetValidBank(uint16 ReadFlag);
...
extern uint16 Erase_Inactive_Unit;
extern uint16 EEPROM_ACTIVE_UNIT;
extern uint16 NUM_EEPROM_SECTORS;
extern uint16 FIRST_AND_LAST_SECTOR[2][2];
