//#############################################################################
//
// FILE:   F28P65x_EEPROM_PingPong.c
//
//#############################################################################


#include "EEPROM_PingPong_Config.h"                   // Include EEPROM Config
...
// User-Configurable EEPROM Variables

// Designate which Bank to begin EEPROM Emulation in
...
uint16 FIRST_AND_LAST_SECTOR[2][2] = {{1,1},{5,5}};
...
int EEPROM_Config_Check(void){
    ...
    // If using Bank 2
    else if (FLASH_BANK_SELECT == FlashBank2StartAddress)
    {
    ...
    // If array indicates two different numbers of sectors for emulation
    ...
    {
        // Check if FIRST_AND_LAST_SECTOR is sorted in increasing order and doesn't have duplicates
    ...
    } else // If only using 1 sector
    {
        ...
        if (FIRST_AND_LAST_SECTOR[0][0] > NUM_FLASH_SECTORS - 1 || FIRST_AND_LAST_SECTOR[1][0] > NUM_FLASH_SECTORS - 1) {
            return 0xEEEE;
        }
    }

    // Check if Sector ranges overlap
        ...
        if (FIRST_AND_LAST_SECTOR[i][1] > 31) {
            ...
            if (FIRST_AND_LAST_SECTOR[i][0] > 31)
            {
                ...
                if (NUM_EEPROM_SECTORS < 8)
                {
                    ...
                    if ((FIRST_AND_LAST_SECTOR[i][0] % 8) != 0 || ((FIRST_AND_LAST_SECTOR[i][1] + 1) % 8 != 0))
                    {
                        Warning_Flags += 4;
                        break;
                    }
    ...
    // Need to erase both EEPROM units before programming, so we need to build WE protection masks that
    // disable protection in all Flash sectors designated for EEPROM emulation

    // Combine sectors from both units and separate them by which protection register they use (A or B)
    uint32 Combined_WE_Protection_A_Sectors = (uint32)WE_Protection_AB_Sectors_Unit_0 | (uint32)WE_Protection_AB_Sectors_Unit_1;
    uint32 Combined_WE_Protection_B_Sectors = WE_Protection_AB_Sectors_Unit_0 >> 32 | WE_Protection_AB_Sectors_Unit_1 >> 32;
...
// This function calculates the bits needed to be set low in Write/Erase Protection masks needed by the Flash API.
...
void EEPROM_GetValidBank(uint16 ReadFlag)
{
    ...
    // Only want to erase when writing to a full EEPROM Bank
    if (!ReadFlag)
    {
        if (Bank_Counter == NUM_EEPROM_BANKS - 1 && Page_Counter == NUM_EEPROM_PAGES)     // Check for full EEPROM
        {
            EEPROM_UpdatePageStatus();              // Update Page Status of previous page
            EEPROM_UpdateBankStatus();              // Update Bank Status of previous page
            EEPROM_ACTIVE_UNIT ^= 1;                // Switch active EEPROM Banks
...
{
    // Re-Configure Write/Erase Protection Masks used by the Flash API
    uint64 WE_Protection_AB_Mask = Configure_Protection_Masks(FIRST_AND_LAST_SECTOR[EEPROM_ACTIVE_UNIT ^ 1], NUM_EEPROM_SECTORS);
...
void EEPROM_UpdatePageData(uint16* Write_Buffer)
{
    ...
    for (i = 0; i < EEPROM_PAGE_DATA_SIZE / 4; i++)
    {
        ...
        // Variable for page offset (first write position has offset of 4 (128 bits),
        // second has offset of 6 (192 bits), etc.)
    ...
    if (Erase_Inactive_Unit)
    {
        ...
        EEPROM_Erase();             // Erase the inactive (full) EEPROM Bank
...
void EEPROM_Program_64_Bits(uint16 Num_Words, uint16 * Write_Buffer)
{
    ...
    // If other bank is full and we just wrote to a new bank
    if (Erase_Inactive_Unit) {

        Erase_Blank_Check = 1;     // Set flag to perform blank check on inactive (full) Flash Bank
        EEPROM_Erase();            // Erase inactive unit
        Erase_Inactive_Unit = 0;   // Reset flag for erasing inactive (full) Flash Bank)
