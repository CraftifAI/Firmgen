//#############################################################################
//
// FILE:   flashapi_cpu2_512bitprogramming.c
//
// TITLE:  Flash programming example
//
//! \addtogroup driver_example_list
//! <h1> Flash Programming with 512-bit AutoECC, DataAndECC, DataOnly and EccOnly </h1>
//!
...
//! 3. DataAndECC
//!
//! \note CPU1 example must be loaded and executed before CPU2 example.
//! Only CPU1 is able to set the BankMuxSel and GSxMSel registers for CPU2.
//! For both CPU1 and CPU2 load, make sure to set the correct Erase settings
//! for bank selection.
//!
...
void Example_CallFlashAPI(void)
{
    ...
    //
     // Erase all bank before programming
     //
     Example_EraseBanks();
     //
     ...
     //
     // Program all Bank using AutoECC option
     //
     ...
     //
     // Program all bank using DataOnly and ECCOnly options
     //
     ...
     //
     // Program all bank using DataAndECC option
     //
...
void Example_ProgramAllBankUsing512bitAutoECC(void)
{
    uint32 u32Index = 0;
     ...
     //
     // A data buffer of max 32 16-bit words can be supplied to the program
     // function.
     // Each word is programmed until the whole buffer is programmed or a
     // problem is found. However to program a buffer that has more than 32
     // words, program function can be called in a loop to program 32 words for
     ...
     // 512-bit address boundaries and each 512 bit word may only be programmed
     // once per write/erase cycle.  Meaning the length of the data buffer
     // (3rd parameter for Example_ProgramAllBankUsing512bitAutoECC() function) passed
     // to the program function can only be 32.
     //
     ...
     //
     // Note that data buffer (Buffer) is aligned on 512-bit boundary for verify
     // reasons.
     //
     // Monitor ECC address for Bank while programming with AutoEcc mode.
     //
     // In this example, Flash Bank  is programmed along with auto-generated
     // ECC.
     //

     // Program Bank 1
     for(sector = Bone_Sector12_start; sector < FlashBank1EndAddress;
     ...
     // Program Bank 2-4
     for(sector = FlashBank2StartAddress; sector < FlashBank4EndAddress;
         sector += Sector2KB_u16length)
     {
         for(i=0, u32Index = sector;
            (u32Index < (sector + WORDS_IN_FLASH_BUFFER));
...
void Example_ProgramAllBankUsing512bitDataOnlyECCOnly(void)
{
    ...
    //
    // Program data using 512-bit "DataOnly" option and 64-bit ECC using "EccOnly" option.
    //
    ...
    // reasons.
    //
    // In this example, Bank 1-4 are programmed
...
void Example_ProgramAllBankUsing512bitDataAndECC(void)
{
    uint32 u32Index = 0;
     ...
     //
     // Program 512-bit data and ECC in Flash using "DataAndEcc" option.
     ...
     // reasons.
     //
     // In this example, Bank1-4 are programmed in Flash
     // along with the specified ECC.
     //
     //Program Bank 1
    for(sector = Bone_Sector12_start; sector < FlashBank1EndAddress;
...
void Example_EraseBanks(void)
{
    ...
    ClearFSMStatus();
     ...
     u32CurrentAddress = FlashBank1StartAddress;
     ...
     u32CurrentAddress = Bone_Sector12_start;
    ...
    // Erase Bank 2, 3 and Bank 4
    for(u32CurrentAddress = FlashBank2StartAddress; u32CurrentAddress < FlashBank4EndAddress; u32CurrentAddress +=FlashBankOffset)
    {
        ClearFSMStatus();
