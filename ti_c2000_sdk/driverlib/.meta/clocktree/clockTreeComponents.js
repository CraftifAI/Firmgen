...
if (("system" === system.context || "CPU1" === system.context) && system.deviceData.clockTree)
{
	...
	function defaultView(ipInstance) {
		...
		if (ipInstance.name == "CANA_CLOCK_domain" || ipInstance.name == "CANB_CLOCK_domain") {
			if (device == "F28P65x" || device == "F2838x" || device == "F2837xD")
			{
				return {
		...
		if (ipInstance.name == "MCANA_CLOCK_domain" || ipInstance.name == "MCANB_CLOCK_domain" || ipInstance.name == "MCANA_domain" || ipInstance.name == "MCANB_domain" || ipInstance.name == "MCAN_domain") {
			if (device == "F2838x")
			{
				return {
					displayName: (ipInstance.description ?? ipInstance.name).replace(/domain/ig,''),
					ipInstances: ["CMCLK", ipInstance.name, "CPU1_SYSCLK", ipInstance.name, "AUXCLKIN", "AUXCLKIN", "AUXRAWCLK", "AUXRAWCLK"],
					algorithm: "everythingBetween",
					frequencyLabels: FreqLabels,
				}
			}
				...
				return {
					displayName: (ipInstance.description ?? ipInstance.name).replace(/domain/ig,''),
					ipInstances: ["PLLRAWCLK", "PLLRAWCLK", "AUXRAWCLK", "MCANABCLKSEL", "AUXCLKIN", "AUXCLKIN", "CPU1_SYSCLK", "MCANABCLKSEL", "CPU2_SYSCLK", "MCANABCLKSEL", "MCANABCLKSEL", ipInstance.name],
					algorithm: "everythingBetween",
					frequencyLabels: FreqLabels,
				}
			}
				...
				return {
					displayName: (ipInstance.description ?? ipInstance.name).replace(/domain/ig,''),
					ipInstances: ["SYSCLK", ipInstance.name, "PLLRAWCLK", "PLLRAWCLK", "AUXCLKIN", ipInstance.name,  ],
					algorithm: "everythingBetween",
					frequencyLabels: FreqLabels,
				}
			}
		}
		//EtherCAT, ETHERNET - include AUXRAWCLK and PLLSYSCLK
		if (ipInstance.name == "EtherCAT_domain" || ipInstance.name == "ETHCLK_domain") {
			return {
				displayName: (ipInstance.description ?? ipInstance.name).replace(/domain/ig,''),
				ipInstances: ["RAWCLK", ipInstance.name, "AUXRAWCLK", ipInstance.name],
				algorithm: "everythingBetween",
				frequencyLabels: FreqLabels,
			}
		}
		//CLB - include AUXRAWCLK or SYSPLLCLK depending on device
		if (ipInstance.name == "CLB_domain") {
			if (device == "F28P65x" || device == "F2838x")
			{
				return {
					displayName: (ipInstance.description ?? ipInstance.name).replace(/domain/ig,''),
					ipInstances: ["AUXRAWCLK", ipInstance.name],
					algorithm: "everythingBetween",
					frequencyLabels: FreqLabels,
				}
			} 
				...
				return {
					displayName: (ipInstance.description ?? ipInstance.name).replace(/domain/ig,''),
					ipInstances: ["PLLRAWCLK", ipInstance.name, "OSCCLK", "OSCCLK"],
					algorithm: "everythingBetween",
					frequencyLabels: FreqLabels,
				}
			} 
		}
		//USB - include AUXRAWCLK
		if (ipInstance.name == "USB_domain") {
			if (device == "F28P55x") {
				...
				return {
					displayName: (ipInstance.description ?? ipInstance.name).replace(/domain/ig,''),
					ipInstances: ["AUXRAWCLK", ipInstance.name, "PLLSYSCLK", ipInstance.name],
					algorithm: "everythingBetween",
					frequencyLabels: FreqLabels,
				}
			}
